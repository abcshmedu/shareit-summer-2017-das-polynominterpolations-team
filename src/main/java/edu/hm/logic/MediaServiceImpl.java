...
=======
	 ...
	 * @param book
	 *            Das Buch, welches hinzugefügt werden soll
	 * @return Liefert ein MediaServiceResult-Objekt zurück, welches
	...
	public MediaServiceResult addBook(final Book newBook) {
		...
		return result;
>>>>>>> branch 'master' of https://github.com/abcshmedu/shareit-summer-2017-das-polynominterpolations-team.git
	}
	...
	public MediaServiceResult addDisc(final Disc newDisc) {
		...
		if (result == null) {
			result = testDiscInStorage(newDisc);
		}
	...
	private MediaServiceResult testDiscInStorage(Disc newDisc) {
		MediaServiceResult result = null;

		if (database.getDisc(newDisc.getBarcode()) != null) {
			result = MediaServiceResult.CONFLICT;
			result.setDetail("Disc with this Barcode already exists.");
		}
