...
public class MediaServiceImpl implements MediaService {
	 ...
	 * 
	 * @param isbn Die ISBN, die getestet werden soll
	...
	private boolean testISBN(final String isbn) {
		...
		if (isbn.contains("-")) {
			...
			if(!(isbn.matches("[0-9]+") && isbn.length() > 3))
				isbnIsCorrect = false;
	 ...
	 * @return Liefert true zurück, falls die/der getestete ISBN/Barcode
	 *         gültig ist. Fals andernfalls.
	 */
